...
-- Adopted `data_services_api` 1.7.0 to pick up per-reason HTTP retry configuration (a longer, more generous retry budget for connection failures, a shorter and more conservative one for timeouts, and `Faraday::ResourceNotFound` no longer retried) [#342](https://github.com/epimorphics/ppd-explorer/issues/342).
